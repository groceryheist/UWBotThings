...
class Controller(object):
    ...
    def getTopHashTags(self, n=None):
        ...
        s = sesh.query(func.lower(HashTag.hashtag), func.count(HashTag.status)).\
            ...
            filter(and_(func.length(HashTag.hashtag) > 4, Twitter_User.istarget == True,Status.created_at != None, Status.created_at > (datetime.now() - timedelta(weeks=2)))).\
...
#        populateHashTag.run()
    ...
    def update_tweetGenerator(self, hashtag):
        ...
        try:
            tg = randomTweetGenerator(hashtag)            
        except Exception as e:
            print str(e)
            return
        self.tweetGenerators[hashtag] = tg
 ...
 #       self.dictLock.release()
    ...
    def getSpamTweet(self):
        ...
        hts = ' '.join(random.sample(ProVaxHashTags + AntiVaxHashTags,2))
        lnk = random.sample(links,1)[0]
        ts = random.sample(ts,1)[0]
        return ts.replace('(link)', lnk) + hts
